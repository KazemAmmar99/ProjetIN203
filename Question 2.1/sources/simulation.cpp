...
void simulation(bool affiche)
{
    ...
    while (!quitting)
    {
        ...
        std::chrono::duration<double> elapsed_seconds_aff = end2 - start2;
        ...
        temps2 += elapsed_seconds_aff.count();
    }
    std::cout << "Temps execution moyen : " << temps / jours_écoulés << std::endl;// Fin boucle temporelle
    std::cout << "Temps affichage moyen : " << temps2 / jours_écoulés << std::endl;// Fin boucle temporelle
    output.close();
}
