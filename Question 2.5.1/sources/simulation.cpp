...
void simulation(bool affiche, int nargs, char *argv[])
{
    ...
    if (rank >= 1)
    {
        ...
        size_t individu_par_processus = rank == nbp - 1 ? contexte.taux_population - (nbp - 1) * k : k;

        float Temps_Global = 0;
        std::vector<épidémie::Individu> population;
        population.reserve(individu_par_processus);

        // L'agent pathogene n'evolue pas et reste donc constant...
        épidémie::AgentPathogène agent(graine_aléatoire++);
        // Initialisation de la population initiale :
        graine_aléatoire += (rank - 1) * k;

        for (std::size_t i = 0; i < individu_par_processus; ++i)
        {
        ...
        int Taux2 = (contexte.taux_population*23) / 100;

        int nombre_immunises_grippe = rank == nbp - 1 ? Taux2 - (nbp - 1) * (individu_par_processus * 23) / 100 : (individu_par_processus * 23) / 100;
        ...
        while (!quitting)
        {
            ...
            if (jours_écoulés % 365 == 0) // Si le premier Octobre (debut de l'annee pour l'epidemie ;-) )
            {
                ...
                #pragma omp parallel for num_threads(2)
                for (int ipersonne = nombre_immunises_grippe; ipersonne < int(individu_par_processus); ++ipersonne)
                {
