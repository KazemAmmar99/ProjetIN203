...
void simulation(bool affiche, int nargs, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        while (!quitting)
        {
            ...
            MPI_Send(&quitting, 1, MPI_INT, 1, 100, globComm);
            MPI_Recv(rcv_buffer.data(), hauteur_grille * largeur_grille, Stat_Type, 1, 0, globComm, &Stat1);
            MPI_Recv(&jours_écoulés, 1, MPI_INT, 1, 1, globComm, &Stat1);
    ...
    if (rank == 1)
    {
        ...
        while (!quitting)
        {
            ...
            if (rank == 1)
            {
                ...
                MPI_Recv(&quitting, 1, MPI_INT, 0, 100, globComm, &Stat2);
                MPI_Send(buffer.data(), largeur_grille * hauteur_grille, Stat_Type, 0, 0, globComm);
                MPI_Send(&jours_écoulés, 1, MPI_INT, 0, 1, globComm);
